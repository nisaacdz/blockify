...
//!
//! A derive macro for the `blockify::record::Record` crate.
//! # Usage
//! ```
...
fn impl_record(input: &DeriveInput) -> proc_macro::TokenStream {
    ...
    let gen = quote! {
        impl #impl_generics blockify::record::Record for #name #ty_generics #where_clause {
            fn sign(&self, key: &blockify::AuthKeyPair) -> Result<blockify::DigitalSignature, blockify::SigningError> {
                let msg = blockify::serialize(self)?;
                let signature = blockify::sign_msg(&msg, key)?;
                Ok(signature)
            }

            fn verify(&self, signature: &blockify::DigitalSignature, key: &blockify::PublicKey) -> Result<(), blockify::VerificationError> {
                let msg = blockify::serialize(self).map_err(|_| blockify::VerificationError::SerdeError)?;
                key.verify(&msg, signature)
            }

            fn record(self, keypair: blockify::AuthKeyPair, metadata: blockify::data::Metadata) -> Result<blockify::record::SignedRecord<Self>, blockify::SigningError> {
